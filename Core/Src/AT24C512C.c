...
#endif
	if (HAL_I2C_IsDeviceReady(i2c_handle, _EEPROM_ADDRESS, 2, MAX_TIMEOUT_MEM)
			== HAL_OK)
		return true;
	else
		return false;
...
#else
		if (HAL_I2C_Mem_Write(i2c_handle, _EEPROM_ADDRESS, address,
		I2C_MEMADD_SIZE_16BIT, data, w, MAX_TIMEOUT_MEM) == HAL_OK)
#endif
				{
			at24_delay(10);
			len -= w;
...
#else
	if (HAL_I2C_Mem_Read(i2c_handle, _EEPROM_ADDRESS, address,
...
bool at24_eraseChip(void) {
	const uint8_t eraseData[32] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
			0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
			0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
			0xFF, 0xFF, 0xFF };
	uint32_t bytes = 0;
	while (bytes < (_EEPROM_SIZE_KBIT * 128)) {
		if (at24_write(bytes, (uint8_t*) eraseData, sizeof(eraseData),
		MAX_TIMEOUT_MEM) == false)
			return false;
...
enum ERROR write_data_to_eeprom(TMP100_DATA *tmp_data, RM3100_DATA *rm_data) {
	...
	at24_write(current_address, (uint8_t*) &id_pkt, sizeof(id_pkt),
	MAX_TIMEOUT_MEM);
	current_address += sizeof(id_pkt);
	id_pkt++;

	// Write temperature value to EEPROM
	at24_write(current_address, (uint8_t*) &tmp_data->temp,
			sizeof(tmp_data->temp), MAX_TIMEOUT_MEM);
	current_address += sizeof(tmp_data->temp);

	// Calculate and write CRC of temperature value to EEPROM
	uint16_t crc_temp = crc16((uint8_t*) &tmp_data->temp,
			sizeof(tmp_data->temp));
	at24_write(current_address, (uint8_t*) &crc_temp, sizeof(crc_temp),
	MAX_TIMEOUT_MEM);
	current_address += sizeof(crc_temp);

	// Write status value to EEPROM
	at24_write(current_address, (uint8_t*) &tmp_data->status,
			sizeof(tmp_data->status), MAX_TIMEOUT_MEM);
	current_address += sizeof(tmp_data->status);

	// Calculate and write CRC of status value to EEPROM
	uint16_t crc_status = crc16((uint8_t*) &tmp_data->status,
			sizeof(tmp_data->status));
	at24_write(current_address, (uint8_t*) &crc_status, sizeof(crc_status),
	MAX_TIMEOUT_MEM);
	current_address += sizeof(crc_status);

	// Write x, y, and z values to EEPROM
	at24_write(current_address, (uint8_t*) &rm_data->x, sizeof(rm_data->x),
	MAX_TIMEOUT_MEM);
	current_address += sizeof(rm_data->x);
	at24_write(current_address, (uint8_t*) &rm_data->y, sizeof(rm_data->y),
	MAX_TIMEOUT_MEM);
	current_address += sizeof(rm_data->y);
	at24_write(current_address, (uint8_t*) &rm_data->z, sizeof(rm_data->z),
	MAX_TIMEOUT_MEM);
	...
	at24_write(current_address, (uint8_t*) &crc_xyz, sizeof(crc_xyz),
	MAX_TIMEOUT_MEM);
	current_address += sizeof(crc_xyz);

	// Write gain value to EEPROM
	at24_write(current_address, (uint8_t*) &rm_data->gain,
			sizeof(rm_data->gain), MAX_TIMEOUT_MEM);
	...
	at24_write(current_address, (uint8_t*) &crc_gain, sizeof(crc_gain),
	MAX_TIMEOUT_MEM);
	current_address += sizeof(crc_gain);

	// Write uT value to EEPROM
	at24_write(current_address, (uint8_t*) &rm_data->uT, sizeof(rm_data->uT),
	MAX_TIMEOUT_MEM);
	...
	at24_write(current_address, (uint8_t*) &crc_ut, sizeof(crc_ut),
	MAX_TIMEOUT_MEM);
	current_address += sizeof(crc_ut);

	// Enviar dados de debug
	char debug_message[200]; // Ajuste o tamanho do buffer conforme necessário
	snprintf(debug_message, sizeof(debug_message), "[EEPROM] Writing Data: "
			"{ ID: %u, "
			"Temp: %.2f, "
			"Status: %d, "
			"X: %ld, "
...
enum ERROR read_data_from_eeprom(TMP100_DATA *tmp_data, RM3100_DATA *rm_data) {
	...
	at24_read(pointer_address, (uint8_t*) &tmp_data->status,
			sizeof(tmp_data->status), MAX_TIMEOUT_MEM);
	pointer_address += sizeof(tmp_data->status);

	// Read and verify CRC of status value from EEPROM
	uint16_t crc_status = 0;
	at24_read(pointer_address, (uint8_t*) &crc_status, sizeof(crc_status),
	MAX_TIMEOUT_MEM);
	pointer_address += sizeof(crc_status);
	if (crc_status
			!= crc16((uint8_t*) &tmp_data->status, sizeof(tmp_data->status))) {
		// Handle error
		return ERROR_FAIL;
	}

	// Read x, y, and z values from EEPROM
	at24_read(pointer_address, (uint8_t*) &rm_data->x, sizeof(rm_data->x),
	MAX_TIMEOUT_MEM);
	pointer_address += sizeof(rm_data->x);
	at24_read(pointer_address, (uint8_t*) &rm_data->y, sizeof(rm_data->y),
	MAX_TIMEOUT_MEM);
	pointer_address += sizeof(rm_data->y);
	at24_read(pointer_address, (uint8_t*) &rm_data->z, sizeof(rm_data->z),
	MAX_TIMEOUT_MEM);
	pointer_address += sizeof(rm_data->z);

	// Read and verify CRC of x, y, and z values from EEPROM
	uint16_t crc_xyz = 0;
	at24_read(pointer_address, (uint8_t*) &crc_xyz, sizeof(crc_xyz),
	MAX_TIMEOUT_MEM);
	...
	at24_read(pointer_address, (uint8_t*) &rm_data->gain, sizeof(rm_data->gain),
	MAX_TIMEOUT_MEM);
	pointer_address += sizeof(rm_data->gain);

	// Read and verify CRC of gain value from EEPROM
	uint16_t crc_gain = 0;
	at24_read(pointer_address, (uint8_t*) &crc_gain, sizeof(crc_gain),
	MAX_TIMEOUT_MEM);
	...
	// Read gain value from EEPROM
	at24_read(pointer_address, (uint8_t*) &rm_data->uT, sizeof(rm_data->uT),
	MAX_TIMEOUT_MEM);
	pointer_address += sizeof(rm_data->uT);

	// Read and verify CRC of gain value from EEPROM
	uint16_t crc_ut = 0;
	at24_read(pointer_address, (uint8_t*) &crc_ut, sizeof(crc_ut),
	MAX_TIMEOUT_MEM);
	...
	char debug_message[200]; // Ajuste o tamanho do buffer conforme necessário
	snprintf(debug_message, sizeof(debug_message), "[EEPROM] Reading Data: "
			"{ ID: %u, "
			"Temp: %.2f, "
			"Status: %d, "
			"X: %ld, "
